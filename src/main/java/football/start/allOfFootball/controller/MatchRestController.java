...
public class MatchRestController {
    ...
    public ResponseEntity<JsonDefault> managerApply(@AuthenticationPrincipal PrincipalDetails user, @RequestBody Long matchId) throws NotExistsMatchException {
        ...
        boolean isAlreadyApply = memberService.isAlreadyApply(ordersList, match);
        if (isAlreadyApply) {
    ...
    // 해야할 일
    // (완료) 1. ScoreResultForm 에서 List로 두팀을 표현했는데 List로 받지말고 하나의 객체로 사용해서 List<List<RecordForm>>에서  List<RecordForm> 형식으로 변경
    ...
    public ResponseEntity<JsonDefault> scoreRecord(@PathVariable Long matchId,
        ...
        List<RecordForm> playList = score.getPlayList();
        System.out.println("playList = " + playList);

        for (RecordForm play : playList) {
            if (!play.isValid()) return ResponseEntity.badRequest().body(new JsonDefault(FAIL, "점수 데이터가 존재하지 않습니다."));
        }
        boolean saveScore = scoreService.saveScore(match, playList);
